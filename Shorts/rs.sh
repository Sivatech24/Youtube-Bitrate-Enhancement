...
# ---------------- CONFIG ----------------
SRC="input.mov"
TMP_SEGMENTS="tmp_segments"         # 30s split
TMP_CHUNKS="tmp_chunks"             # 5s chunks
...
OUT_H=7680
FPS=60
PIX_FMT="p010le"                    # 10-bit 4:2:0 (GPU safe)
PROFILE="main10"
PRESET="p7"
CQ=10
# HDR10 metadata
...
# ---------------- STEP 2: Process each 30s segment in GPU-safe 5s chunks ----------------
for SEG in "$TMP_SEGMENTS"/clip_*.mov; do
    ...
    # ---------------- STEP 3: Concatenate encoded 5s chunks ----------------
    ...
    for FINAL_CHUNK in "$TMP_CHUNKS"/${BASE}_part_*_encoded.mov; do
        if [[ -f "$FINAL_CHUNK" ]]; then
            ...
            echo "⚠️ Warning: $FINAL_CHUNK not found!"
        fi
    done
