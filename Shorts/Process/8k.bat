...
REM ---------------- CONFIG ----------------
set "SRC=input.mov"
set "OUT_FOLDER=ETS2_8K_VERTICAL_444_10bit_NVENC_1G"
...
set "PROFILE=rext"
set "PRESET=p6"         REM use p6 to reduce GPU memory pressure
...
for %%F in (tmp_segments\clip_*.mov) do (
    ...
    ffmpeg -y -hwaccel cuda -i "%%F" ^
      ...
      -c:a copy ^
      "%OUT_FOLDER%\!BASE!_8K60_vertical_444_10bit_1G.mov"

    REM Pause 10s to free GPU memory
    timeout /t 10 >nul
)

echo ✅ Done. All 8K60 vertical HDR10 clips saved in: %OUT_FOLDER%
pause
